...
def getdb(dbname, tablename, whereclause= None, condition = None, select = "*", orderby = None, like = None):
    ...
    if condition :
        ...
        shows = cur.execute(f"SELECT {select} FROM {tablename} WHERE {whereclause} LIKE ?", (like,))
    else:
...
# tvshow()
# deletedb("tvEpisodes.db", "tvEpisodes", "Channel_Name", "STAR PLUS")

dblist = getdb(dbname="tvEpisodes.db", tablename="tvEpisodes", whereclause="Show_Name", like='Kasme%')
