...
def get_shows(showurl):
    ...
    for showList in showLists:
        ...
        li.setArt({
            'thumb' : poster if requests.get(poster).status_code == 200 else icon,
            'icon' : poster if requests.get(poster).status_code == 200 else icon,
            'fanart' : backdrop if requests.get(backdrop).status_code == 200 else fanart
        })
...
def get_VideoLink(vidurl):
    log(message="The play episode url is", find=vidurl)
    ...
    def workingLink(search_text):
        ...
        links = videoLinkSoup.find(lambda tag: get_tag(tag, search_text))
        if links:
            links = links.parent.find_next_sibling('p').select_one('a')['href']
            linkSoup = soupObject(links, '#content')
            iframe = linkSoup.select_one('iframe').get('src')
            stream_Url = resolve_url(iframe, subs=True)
            if stream_Url:
                stream_Url = stream_Url.get('url')
                if "master.txt" in stream_Url:
            ...
            log(message="link hi nahi mila", find=vidurl)
            return None
